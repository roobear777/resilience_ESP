...
static String webSetupLiveOutputLabel() {
  return ledDirectOutputRealOutputAllowed() ? "ON" : "SIM ONLY";
}
...
static String webSetupJsonStatus() {
  ...
  json += ledDirectOutputRealOutputAllowed() ? "true" : "false";
  json += ",\"realOutputStarted\":";
  json += ledDirectOutputRealOutputStarted() ? "true" : "false";
  json += ",\"mode\":\"";
  json += ledDirectOutputModeName();
  json += "\",\"lanesOnLcd\":";
  json += String(ledDirectOutputLanesOnLcd());
  json += ",\"laneCount\":";
  json += String(ledDirectOutputConfiguredLaneCount());
  json += ",\"showMs\":";
  json += String(ledDirectOutputAverageShowMs(), 1);
  json += ",\"fps\":";
  json += String(ledDirectOutputFps(), 1);
...
static String webSetupHtmlPage() {
  ...
  html += ledDirectOutputRealOutputStarted() ? "YES" : "NO";
  html += F("</span></div><div class=\"row\"><span class=\"k\">Lanes</span><span class=\"v\">");
  html += String(ledDirectOutputLanesOnLcd());
  html += F(" / ");
  html += String(ledDirectOutputConfiguredLaneCount());
  html += F(" LCD, ");
  html += String(ledDirectOutputFps(), 1);
  html += F(" fps");
